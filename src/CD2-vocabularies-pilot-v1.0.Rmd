...
In this R markdown document, we analyze input from the CD2 cohort representatives during the pilot of the vocabularies survey. This input may also be used for generating the vocabularies that we will use in the data portal from the CD2 project (Connecting Data in Child Development).

Important differences between the pilot and the official version of the survey:
1. The keywords are asked in a different format: one field per keyword, instead of 1 field for all keywords (comma-separated)
2. The response options in the Categories question are different in the pilot version. This can make combining the results of the pilot with the official results a bit more difficult.
...
- For each cohort, a list of numbers, names and descriptions of the measures as used in the Qualtrics loop and merge blocks (located in `docs/instrumentnrs_pilot`)
...
```{r parameters}
# Files to be read in
datafile <- file.path("../data/raw/CD2_vocabularies_PILOT_20220421.csv")

# Cohorts: same order as instrument numbers files
cohorts <- c("TRAILS", "GenR", "RADAR", "LCID", "YOUth", "NTR")

categorynames <- c("Demographics",
                   "Health",
                   "Lifestyle",
...
cd2survey <- cd2survey[,-c("StartDate","EndDate","Status", 
                           ...
                           "UserLanguage")] # removed  "RecordedDate"
```

### Structure of the data file

The data file consists of roughly 3 types of variables that are repeated for all cohorts and measures in that cohort, using Qualtrics's Loop and Merge functionality:

1. Keywords question: which keywords would the respondent apply to the measure in question? <br>Format: `[instrument_number]_[cohort]_Keywords_[fieldnr]`: Keywords string responses for each keyword provided.
2. Categories: which categories would the respondent use for the measure in question? <br>Format: `[instrument_number]_[cohort]_Cat_[category-number]`
3. Category custom categories: if the respondent thinks a category for the measure in question is missing, they can provide up to 3 custom categories. <br>Format: `[instrument_number]_[cohort]_Cat_1[2/3/4]_TEXT`: string indicating the custom category.
4. Category ranking: how would the respondent rank the categories provided for the measure in question? <br>Format: `[instrument_number]_[cohort]_Cat_rank_[category-number]`: the response is a number delineanating the priority given to the category.

## 2. Data exploration

There are `r dim(cd2survey)[1]` respondents for the pilot survey. It took them a median of `r round(median(cd2survey$Duration)/60,1)` minutes to complete the survey (ranging from `r round(min(cd2survey$Duration)/60,1)` to `r round(max(cd2survey$Duration)/60,1)` minutes). This large range is possibly due to the fact that not all respondents finished the survey: the completion rate is on average `r mean(cd2survey$Progress)`% (SD = `r sd(cd2survey$Progress)`).
...
# Prereqs: separate all words in a list from the Expertise column, count each occurrence. Result should be: dataframe with columns word and freq
expertise <- 
...
# save it in html
#library("htmlwidgets")
#saveWidget(wordcloudexpertise,"tmp.html",selfcontained = F)

# and in png or pdf
#webshot("tmp.html","wordcloud_expertise.png", delay =5, vwidth = 480, vheight=480)
```
...
instrumentnrsfiles <- c("instrumentnrs_pilot/TRAILS_instrumentnrs_pilot.csv", 
                        "instrumentnrs_pilot/GenR_instrumentnrs_pilot.csv", 
                        "instrumentnrs_pilot/RADAR_instrumentnrs_pilot.csv", 
                        "instrumentnrs_pilot/LCID_instrumentnrs_pilot.csv", 
                        "instrumentnrs_pilot/YOUth_instrumentnrs_pilot.csv", 
                        "instrumentnrs_pilot/NTR_instrumentnrs_pilot.csv")
...
for (cohort in 1:length(instrumentnrs)) {
  ...
  instrumentnrs[[cohort]]$Instrument_description <- 
    as.character(instrumentnrs[[cohort]]$Instrument_description)
...
for (cohort in 1:length(cohorts)) {
  ...
  for (instrument in 1:length(keywords_count_tempname)) {
    ...
    # Select all keywords belonging to one instrument (they are separated in multiple columns)
    keywords_current_instr <- rawkeywords[, grep(paste0("^", instrument, "_"),
                                                 names(rawkeywords), value = TRUE),
                                          with = FALSE]
    
    # Combine all keyword columns from this instrument into 1 column, comma-separated
    keywords_current_instr_combined <- unite(keywords_current_instr, "combined",
                                             sep = ",", remove = TRUE, na.rm = TRUE)
    
    # Create a character vector of the keyword for this instrument
    keyword <- str_trim(unlist(str_split(keywords_current_instr_combined$combined, 
                                         ","), 
                               use.names = FALSE))
    
    # Remove the NAs and -99s, lower case all
    keyword <- keyword[keyword != "-99" & !is.na(keyword) & keyword != ""]
  ...
  for (instrument in 1:length(keywords_count_tempname)) {
    ...
    # OLD: this code does not work for the current data format, skipping
...
for(cohort in 1:length(cohorts)){
  ...
  for(listitem in 1:length(categoriesprior_raw)){
    ...
    # Select the category ranking variables from the raw dataset for each cohort and list item
    x <- cd2survey[, grep(paste0("^", listitem, "_", cohorts[cohort], "_",catrankvar), 
    ...
    ## RESULT: list variable categoriesprior_raw containing for each measure in the cohort the raw priority scores
      ...
      if(length(categoriesprior_raw[[listitem]][[column]]) != 0){
        ...
        if(column <= length(categorynames)){
          ...
          names(categoriesprior_raw[[listitem]])[column] <- paste0("other_",
                                                                   sub(paste0(".*", cohorts[cohort],"_",catrankvar), "",
    ...
    ## RESULT: column names in each list item (measure) correspond to category names
    
    # Put the TEXT variables (custom category names) in a separate list. 
    textvars <- cd2survey[, grep(paste0("^", listitem, "_", cohorts[cohort], 
                                        "_", catvar,"1[0-9]_TEXT"),
                                 names(cd2survey), value = TRUE), with=FALSE]
  ...
  for(instrument in 1:length(categoriesprior_rel)){
    ...
    for(row in 1:dim(categoriesprior_rel[[instrument]])[1]){ # For each participant
      ...
      # in nrparticipants (for group average)
...
write.csv(results_TRAILS, file.path(paste0(results_folder, "/", todaysdate, 
                                           "_pilotresults_TRAILS.csv")), row.names = TRUE)
