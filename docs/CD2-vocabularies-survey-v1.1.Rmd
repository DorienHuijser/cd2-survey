...
```{r parameters}
# Files to be read in
datafile <- "../data/raw/CD2_vocabularies_22020224.csv"
...
for(cohort in 1:length(cohorts)){
  ...
  customcategories <- vector(mode = "list", 
                             length = length(instrumentnrs[[cohort]][, 1]))
  noncustomaverage <- vector(mode = "list", 
                             length = length(instrumentnrs[[cohort]][, 1]))
  customcols <- vector(mode = "list", 
                       length = length(instrumentnrs[[cohort]][, 1]))
  customaverage <- vector(mode = "list", 
                          length = length(instrumentnrs[[cohort]][, 1]))
  averageprioritieslist <-  vector(mode = "list",
                                   length = length(instrumentnrs[[cohort]][, ]))
  ...
  for (listitem in 1:length(categoriesprior_raw)) {
    ...
    print(paste0("Working on cohort: ", cohorts[cohort], 
                 ". Instrument: ", measurenames[listitem]))
    ...
    textvars <- cd2survey[, grep(paste0("^", listitem, "_",cohorts[cohort], 
                                        "_", catvar,"1[0-9]_TEXT"), names(cd2survey), 
  ...
  for (instrument in 1:length(categoriesprior_rel)) { # For each instrument
    ...
    otherscores <- categoriesprior_rel[[instrument]][,
                                                     grepl("other", 
                                                           names(categoriesprior_rel[[instrument]])), 
                                                     with = FALSE]
    
    # Combine the customcategories (text) and average priorities (numeric) dataframes 
    # in the customcols list 
    ...
    if (length(customcols[[instrument]] != 0)) { 
      ...
      # For each unique categoryname, calculate the average priority 
      # and assign it to customaverage
      ...
      for (uniqueitem in 1:length(categoryname)) {
        ...
        # Divide by nr of people who assessed the instrument, 
        # not the total nr of participants
      ...
      # assign usable names
      names(customaverage)[instrument] <- paste0(instrument, "_",
                                                 measurenames[instrument]) 
    ...
    averageprioritieslist[[instrument]] <- rbind(noncustomaverage[[instrument]],
                                                 customaverage[[instrument]])
    # assign usable names
    names(averageprioritieslist)[instrument] <- paste0(instrument, "_",
                                                       measurenames[instrument]) 
