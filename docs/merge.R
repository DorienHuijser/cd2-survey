...
options(scipen=999) # Convert scientific notation (-e) to float in R
# setwd("/CID-2/data/processed")
...
for (i in 1:length(cohort.df)){
  cohort.df[i]<-paste0("2022-04-07_results/2022-04-07_results_", cohort.df[i],".csv") }

svylist<- lapply(cohort.df, function(data){read.table(data, header=T, sep = ",") })
names(svylist)<-cohort.names

svylist.names<-names(svylist[[1]])[-2]
for (j in 1:length(svylist.names)){svylist.names[j]<-paste0(svylist.names[j],".svy") }

for (i in 1:length(svylist)){
  svylist[[i]]$Instrument_name<-trimws(svylist[[i]]$Instrument_name, which = "both") #removes spaces before and at the end of the name to match with datataxonomie
  
for(j in 1:length(svylist[[i]]$Instrument_name)){
  svylist[[i]][j,"Instrument_name"]<-str_split(svylist[[i]][j,"Instrument_name"], "_")[[1]][2] }# removes "number_"
  svylist[[i]]$svy<-"svy"
}
...
cohort.df<-cohort.names
for (i in 1:length(cohort.df)){
  cohort.df[i]<-paste0("2022-04-21_pilot_results/2022-04-21_pilotresults_", cohort.df[i],".csv") }
pilotlist<- lapply(cohort.df, function(data){read.table(data, header=T, sep = ",") })
names(pilotlist)<-cohort.names


pilotlist.names<-names(pilotlist[[1]])[-2]
for (j in 1:length(pilotlist.names)){pilotlist.names[j]<-paste0(pilotlist.names[j],".pilot") }

for (i in 1:length(pilotlist)){
  pilotlist[[i]]$Instrument_name<-trimws(pilotlist[[i]]$Instrument_name, which = "both") #removes spaces before and at the end of the name to match with datataxonomie
  
  for(j in 1:length(pilotlist[[i]]$Instrument_name)){
    pilotlist[[i]][j,"Instrument_name"]<-str_split(pilotlist[[i]][j,"Instrument_name"], "_")[[1]][2] }# removes "number_"
...
names(svylist[[i]])<-c(names(svylist[[i]])[1], svylist.names, names(svylist[[i]])[11], pilotlist.names, tail(names(svylist[[i]]),1))

svylist[[i]]$assessed <-"both"
svylist[[i]][which(svylist[[i]]$svy=="svy" & is.na(svylist[[i]]$pilot)),  ]$assessed<-"svy only"
svylist[[i]][which(is.na(svylist[[i]]$svy) & svylist[[i]]$pilot=="pilot"),]$assessed<-"pilot only"
}

cohort.df<-cohort.names
for (i in 1:length(cohort.df)){
  cohort.df[i]<-paste0("merged_", cohort.df[i], ".csv") }

for (i in 1:length(svylist)){
  write.csv(svylist[i], cohort.df[i], row.names=FALSE)
} 
